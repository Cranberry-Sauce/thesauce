...
export default function UserTable() {
  ...
  function handleShowVerification() {
    if (!clicked) {
      axios.post('/api/verification').then((data) => {
  ...
  const getClassNamesFor = (name) => {
    ...
    if (sortConfig.key === name) {
      return sortConfig.direction === 'ascending'
        ...
        : 'ml-2 flex-none rounded bg-gray-200 text-gray-900 group-hover:bg-gray-300 -rotate-180';
    } else {
      return 'invisible ml-2 flex-none rounded text-gray-400 group-hover:visible group-focus:visible';
    }
  };
                    ...
                    <thead className='bg-gray-50'>
                      <tr>
                        <th
                          ...
                          onClick={() => handleSearch('first_name')}
                        >
                        ...
                        <th
                          ...
                          onClick={() => handleSearch('cohort_num')}
                        >
                          <a href='#' className='group inline-flex'>
                            Cohort
                        ...
                        <th
                          scope='col'
                          className='px-3 py-3.5 text-left text-sm font-semibold text-gray-900'
                          onClick={() => handleSearch('resident_alum')}
                        >
                          <a href='#' className='group inline-flex'>
                            Resident or Alumni
                            <span
                              className={getClassNamesFor('residentOrAlum')}
                            >
                              <ChevronDownIcon
                                className='h-5 w-5'
                                aria-hidden='true'
                        ...
                        <th
                          ...
                          className='px-3 py-3.5 text-left text-sm font-semibold text-gray-900'
                        >
                          <a className='group inline-flex'>Socials</a>
                        </th>
                      </tr>
                    </thead>
                    <tbody className='divide-y divide-gray-200 bg-white'>
                      {sortedUsers.map((user) => (
                        <tr key={user._id}>
                          ...
                          {user.resident_alum === 'alumni' && (
                            <td className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>
                              <span className='inline-flex rounded-full bg-yellow-100 px-2 text-xs font-semibold leading-5 text-yellow-800'>
                                {user.resident_alum[0].toUpperCase() +
                                  user.resident_alum.slice(1)}
                              </span>
                            </td>
                          )}
                          {user.resident_alum === 'resident' && (
                            <td className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>
                          ...
                          {!user.showsalary && (
                            <td className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>{`Private`}</td>
                          )}
                          <td className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>
                            {user.linkedin && (
                              <a
